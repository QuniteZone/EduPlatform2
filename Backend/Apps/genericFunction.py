...
import fitz
import openai
from Backend.Apps.ragflow_operations import RAGflow
from Backend.Apps.config import model,temperature,ragflow_BASE_URL,ragflow_API_KEY,LLMs_ALLOWED_IMAGE_EXTENSIONS,LLMs_ALLOWED_FILE_EXTENSIONS,LLMs_model
...
- **教学主题**：{课程的主题或核心知识点}  
- **适用情境**：面向{输入年级}年级的{输入科目}课堂，适用于{输入具体教学阶段，例如：初学阶段、深入探讨阶段等}，帮助学生理解{输入核心知识点或技能}  
- **教学内容**：包括{输入教学内容的核心概念或知识点}，同时涵盖其实际应用或具体实例，引导学生将理论与实践结合  
- **教学目标**：  
  输入本节课希望学生达到的预期学习成果，包括知识、能力、情感等方面
  - 理解{输入教学内容}的基本概念与要点  
  - 能够在实际生活或课堂学习中应用{输入教学内容}  
  - 培养学生的{输入关键能力，如批判性思维、问题解决能力、合作能力等}  
...
- **教学程序**：  
给出一个具体的可执行的脚本。例如，
  - 通过引导学生思考与生活中相关的现象或问题来激发兴趣，如：“你们在日常生活中有遇到过{相关问题或现象}吗？”  
  - 提问引导或播放短视频、展示图片等，帮助学生引入{输入教学内容}的相关背景或知识
  - ...
...
- **教学程序**：  
给出一个具体的可执行的脚本。例如，
  - 学生通过课本、网络资源或教师提供的材料，进行自主学习，深入理解{输入教学内容}  
  - 学生思考并记录重要问题，如：  
    1. {知识点}的定义是什么？  
    2. 它如何在实际中得到应用？  
    3. 学生认为掌握这一知识的意义何在？  
...
- **教学程序**：  
给出一个具体的可执行的脚本。例如，
  - 教师提供相关的案例或情境（如：“在{相关领域}中，如何运用{知识点}来解决实际问题？”），并引导学生分析  
...
给出一个具体的可执行的脚本。例如，
  - 任务一：完成与本课相关的练习或思考题  
  - 任务二：选择一个实际案例，分析{输入知识点}在其中的应用，并准备报告或展示材料
  - ...
...
## 三、素养目标
给出一个具体的内容。例如，
- **学科素养**：理解并掌握{输入知识点}的核心概念，能够在实际问题中进行有效应用  
