...
class StudentClusterModel:
    ...
    def prepare_features(self, student_ids: List[str], current_date: str) -> pd.DataFrame:
        ...
        for student_id in student_ids:
            try:
                data, TagResults=get_user_profile(student_id, current_date)
                # profile = data.iloc[0].to_dict()
                profile = data #返回对象已经是 dict 无需额外处理
    ...
    def load_model(cls):
        """从指定路径加载模型"""
        model = cls()
        pkl_path = os.path.join('static', 'student_ML_tag.pkl')
...
def get_user_profile(student_id,data_time):
    # 查询学生基础信息
    def get_student_info(student_id):
        ...
        if studentData:
            ...
            scholl=None
            print(f"没有找到学生信息")
        return faculty, class_name, major,scholl
    ...
    ##获取学生的信息 ##
    faculty, class_name, major,school = get_student_info(student_id)
    # 获取学生的答题answerLog记录
